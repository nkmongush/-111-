...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const dateDiv = document.getElementById('date');
    const genderDiv = document.getElementById('gender');
    const registrationModal = document.getElementById('registrationModal'); // Получаем модальное окно
    const registrationForm = document.getElementById('registrationForm');
    ...
    function updateProfileDisplay() {
        ...
        if (registrationData) {
            ...
            profileSlide.style.display = 'none'; // Скрываем профиль, если нет данных
        }
    }

    //Вызываем updateProfileDisplay, чтобы данные отображались при загрузке profile.html
    if (currentPath.includes('profile.html')) {
        updateProfileDisplay();
    }
    ...
    registrationForm.addEventListener('submit', function (event) {
        ...
        if (name && date && gender) {
            ...
            updateProfileDisplay(); // Обновляем профиль после успешной регистрации
    ...
    // Отображаем модальное окно регистрации при первой загрузке сайта
    if (registrationModal && !localStorage.getItem('hasVisited')) {
    ...
    const closeModalButton = registrationModal.querySelector('.close-modal');
    if (closeModalButton) {
    ...
    window.addEventListener('click', function (event) {
        if (event.target === registrationModal && registrationModal) {
            registrationModal.style.display = 'none';
        }
    });
});